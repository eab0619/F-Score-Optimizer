...
#super high name variation rate isr benchmark
#need to drop duplicates for id2 to remove records where there is no name variation
dfA = isr_noisy.loc[isr_noisy.unique_id.isin(id2)].drop_duplicates(subset=["unique_id", "name", "homonomy", "var_count"]).groupby("unique_id").apply(lambda x: x.sample(1, random_state=1))
dfA = pd.concat([dfA,isr_noisy.loc[isr_noisy.unique_id.isin(id1)].copy()]).reset_index(drop=True)
dfA["file"] = "A"


dfB = isr_noisy.loc[isr_noisy.unique_id.isin(id2)].drop_duplicates(subset=["unique_id", "name", "homonomy", "var_count"]).groupby("unique_id").apply(lambda x: x.sample(1, random_state=2)).reset_index(drop=True)
dfB["file"] = "B"
...
combined.to_csv("../data-raw/isr_benchmark-2.tsv", sep="\t", index=False)
