...
```{r}
testing <- simulate_BRL(m = list(c(5000, 1000),
                                  c(5000, 1000),
                                  c(5000,1000)
                                  ),
                         u = list(c(500,5000),
...
testing <- simulate_BRL(m = list(c(5000, 100),
                                  c(5000, 100),
                                  c(5000,100)
                                  ),
                         u = list(c(1000,5000),
...
```
## problematic case with F score optimizer
```{r}
testing <- simulate_BRL(m = list(c(5000, 1000),
                                  c(5000, 1000),
                                  c(5000,1000)
...
m_1 = MCMCpack::rdirichlet(1, c(1000, 0))
m_2 = MCMCpack::rdirichlet(1, c(1000,0))
m_3 = MCMCpack::rdirichlet(1, c(1000,30))

u_1 = MCMCpack::rdirichlet(1, c(1000,5000))
u_2 = MCMCpack::rdirichlet(1, c(1000,5000))
u_3 = MCMCpack::rdirichlet(1, c(400,5000))
...
id = comp_vec %>% filter(match_ind==T) %>% pull(id)
#sample the comparison vectors for m
col1 = rmultinom(overlap_size, 1, c(m_1)) %>% t()
col2  = rmultinom(overlap_size, 1, c(m_2)) %>% t()
col3 = rmultinom(overlap_size, 1, c(m_3)) %>% t()

combined_m = cbind(id, col1, col2, col3)


id = comp_vec %>% filter(match_ind==F) %>% pull(id)

# sample the comparison vectors for u
col1 = rmultinom(nrow(comp_vec) - overlap_size, 1, c(u_1)) %>% t()
col2  = rmultinom(nrow(comp_vec) - overlap_size, 1, c(u_2)) %>% t()
col3 = rmultinom(nrow(comp_vec) - overlap_size, 1, c(u_3)) %>% t()

combined_u = cbind(id,col1, col2, col3)
...
## set up additional fields for BRL Bipartite
```{r}
file1 <- c("fname", "lname", "city")
file2 <- c("fname", "lname", "city")
types <- c("bi", "bi", "bi")
...
chain <- BRL::bipartiteGibbs(cd = res, nIter = 10000)
...
overlap <- apply(chain_final, 2, function(x){sum(x<=n1)})
  ...
  #measure time take for F-Optim function
  start.time <- Sys.time()
  check <- optim_F(tableLabels, iteration=10000, B=1)
  end.time <- Sys.time()
  time.taken <- round(end.time-start.time, 2)
  print(paste0("The time taken for F-Score Optimizer is: ", time.taken))
...
```{r}
test <- cbind(match_df,check$z, bayes)
```


```{r}
sum(test$true_index==test$`check$z`)
sum(test$true_index==test$bayes)
```
